    ...
    class "_Async" {
        ...
        iterate = function(self, from, to, func, callback)
            self:add(function()
                ...
                if (self.debug) then
                    print("[marlon_async]): " .. (getTickCount() - a) .. "ms");
                end
                if (callback) then
                    callback();
                end
        ...
        -- Usage:
            ...
            --     print(vehicle.title);
            -- end);
        foreach = function(self, array, func, callback, tipo)
            if (not tipo or tipo == 'ipairs') then
                self:add(function()
                    local a = getTickCount();
                    local lastresume = getTickCount();
                    ...
                    if (self.debug) then
                        print("[marlon_async]: foreach: " .. (getTickCount() - a) .. "ms");
                    end
                    if (callback) then
                        callback();
                    end
                end);
            elseif (tipo == 'pairs') then
                self:add(function()
                    local a = getTickCount();
                    local lastresume = getTickCount();
